...
function cell(g,x,y,c,a=1,size=CELL){g.save();g.globalAlpha=a;if(c===WILD){let grad=g.createLinearGradient(x*size,y*size,(x+1)*size,(y+1)*size);grad.addColorStop(0,'#36c978');grad.addColorStop(.5,'#9b6de3');grad.addColorStop(1,'#ff963d');g.fillStyle=grad;g.shadowColor='#fff';g.shadowBlur=7;}else g.fillStyle=COLORS[c];g.fillRect(x*size+2,y*size+2,size-4,size-4);if(c===WILD){g.fillRect(x*size+2,y*size+2,size-4,size-4);g.strokeStyle='#fff';g.lineWidth=2;g.strokeRect(x*size+4,y*size+4,size-8,size-8)}g.restore()}
